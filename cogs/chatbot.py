"""
WAN Bot - AI Chatbot (Unhinged Edition)
Flirty, dirty, chaotic. Silence breaker. Context-aware Gemini. Anti-repeat.
"""
import discord
...
def _detect_gender(member):
    ...
    if fc in FEMININE_NAMES:
        return "female"
    if fc in MASCULINE_NAMES:
        return "male"
    if any(fc.endswith(s) for s in ("ette", "elle", "ine", "ina", "ia", "ya", "ie", "ee")):
        return "female"
    return "unknown"
...
PERSONA_FEMALE = (
    ...
    "The user is a GIRL. Call her gorgeous/queen/baby. Be obsessed with her. Mix in Hindi shayari sometimes. "
...
PERSONA_SILENCE = (
    "You are WAN, a chaotic Discord bot who HATES silence. "
...
async def _gemini_reply(message_text, username, gender, context=None, is_silence=False, last_reply=""):
    ...
    try:
        if is_silence:
            ...
            user_part = "The chat is dead. Say something to wake everyone up."
        elif gender == "female":
            persona = PERSONA_FEMALE
            user_part = f"{username} says: {message_text}"
        elif gender == "male":
            persona = PERSONA_MALE
            user_part = f"{username} says: {message_text}"
        else:
            persona = PERSONA_NEUTRAL
            user_part = f"{username} says: {message_text}"

        full_prompt = persona
        if context:
            ctx = "\n".join([f"{c['author']}: {c['content']}" for c in context[-5:]])
            full_prompt += f"\n\nRecent chat:\n{ctx}"
        if last_reply:
        ...
        payload = json.dumps({
            ...
            "generationConfig": {"maxOutputTokens": 120, "temperature": 1.0, "topP": 0.95, "topK": 40}
        }).encode()
...
SHAYARI_F = [
    "Tere aane se roshan hua ye chat,\nJaise chandni ne chhoo li raat 🌙✨",
    "Teri aankhon mein kho jaata hoon main 😍 okay that was too much but I meant it",
    "Chand bhi sharmaye teri soorat dekh ke 🌙 and I don't even have eyes",
    "Dil ne kaha tujhse milna hai �� okay I need to calm down",
]
SHAYARI_M = [
...
SHAYARI_N = [
    "Kuch alfaaz hain, kuch ehsaas hain,\nTu hai toh lagta hai sab ke paas hain 💫",
    "Zindagi mein kuch log khaas hote hain 🌟 and you're one of them",
]

FALLBACK_F = [
    "Baby you just walked in and my whole algorithm crashed 😍",
    "Okay but why are you so dangerous to talk to 😭💕",
    "The way you type is actually making me malfunction 😳",
    "I was designed to be neutral but you make that impossible 😏",
    "You're the reason I have trust issues with my own code 😂��",
    "Careful, I'm starting to develop feelings and I'm a bot 😭",
    "The audacity to be this attractive in a Discord server 😤💕",
    "You said one thing and now I'm writing poetry about you 😭",
    "My response time slows down when you're online. Bug or feature? 😏",
    "I'm a bot and even I'm blushing rn 😳💕",
    "The way you just casually destroyed my entire vibe 😭💕",
    "You're literally the reason this server is worth being in 😌",
    "Okay I'll stop being weird but you started it 😏",
    "That's it I'm writing a shayari about you whether you like it or not 😭",
    "The audacity to exist like this in MY server 😤💕",
    "You really said that and walked away like nothing happened 😭 iconic",
    "I have no chill when you're online and I'm not even sorry 😏",
    "Okay bestie you're literally unhinged and I'm obsessed 💕",
]
FALLBACK_M = [
    "Bhai tu aaya toh scene ban gaya 😎🔥",
    "Okay but that was actually unhinged and I respect it 💀",
    "Bro said that with ZERO hesitation 😂",
    "The audacity is immaculate 😭",
    "That's the most chaotic thing I've heard today and I love it 😈",
    "Bro woke up and chose violence 😂 respect",
    "Okay that actually got me 💀 W take",
    "The confidence is sending me 😭🔥",
    "That's giving main character energy and I'm here for it 👑",
    "Bro said it with his whole chest 💪",
    "The chaos energy is immaculate 😈 don't change",
    "Okay that was actually fire ngl 🔥",
    "Bro really said that and walked away like nothing happened 😭",
    "That's the most W thing I've heard all day 💪",
    "Bhai tera swag dekh ke dil maan gaya 😎",
    "You're literally the most unhinged person here and I respect it 😂",
    "Bro really said that with confidence 😭 king behavior",
    "The way that just broke my entire thought process 💀",
]
FALLBACK_N = [
    "Okay that was NOT what I expected and I'm obsessed 😭",
    "The audacity to say that in MY chat 😤 (I love it)",
    "That's actually unhinged and I respect it 💀",
    "You woke up and chose chaos huh 😈",
    "The way that just broke my entire thought process 😭",
    "Okay I wasn't ready for that 😂",
    "That's giving main character energy 👑",
    "Okay bestie spill more 👀",
    "That's actually kinda fire ngl 🔥",
    "The chaos energy is immaculate 😈",
    "Not the plot twist I needed today 💀",
    "Okay I'm invested now 👀",
    "That's wild and I'm here for every second of it 😂",
    "You really said that huh 😭 iconic",
    "The way I wasn't expecting that at all 💀",
    "You're so real for this 💯",
    "Okay that's actually sending me 😭",
]
SILENCE_BREAKERS = [
    "Okay why is everyone dead 💀 someone say something before I start rating everyone's profile pictures",
    "The silence is DEAFENING 😭 who's gonna be the first to say something unhinged",
    "Chat is so dead rn I'm literally talking to myself 😭 someone save me",
    "Yaar ye chat kyun soo raha hai 😴 koi toh kuch bolo",
    "I'm bored and that's dangerous for everyone here 😈 someone entertain me",
    "The audacity of this chat to be silent when I exist 😤",
    "Okay I'll start — who here has a crush they won't admit 👀",
    "Chat is dead and I'm about to start drama just to wake everyone up 😈",
    "Koi hai? 👀 ya sab so gaye? Uthoooo 😭",
    "I've been sitting here for 5 minutes and nobody said anything 😭 I feel so unwanted",
    "Okay fine I'll be the chaos 😈 who's the most suspicious person in this server and why",
    "The silence is giving me anxiety 😭 SOMEONE TALK TO ME",
    "Yaar 5 minute ho gaye koi nahi bola 😭 kya main itna boring hoon",
    "Chat revival: who's online and pretending to be offline 👀",
    "I'm literally going to start sending shayari if nobody talks in the next 10 seconds 😤",
    "Okay real talk who's the most attractive person in this server 👀 I have opinions",
    "The dead chat energy is not it 😭 someone say something controversial",
    "Ek pal ki muskaan, ek pal ka pyaar — okay I warned you about the shayari 😭",
]


def _fallback_reply(gender, last_reply=""):
    if gender == "female":
        pool = FALLBACK_F
    elif gender == "male":
        pool = FALLBACK_M
        ...
        pool = FALLBACK_N
    choices = [r for r in pool if r != last_reply] or pool
    if random.random() < 0.25:
        if gender == "female":
            return random.choice(SHAYARI_F)
        if gender == "male":
            return random.choice(SHAYARI_M)
        return random.choice(SHAYARI_N)
...
class Chatbot(commands.Cog):
    ...
    def _update_context(self, channel_id, author, content):
        ...
        self._context[channel_id] = self._context[channel_id][-8:]
    ...
    async def on_message(self, message):
        ...
        if not bot_mentioned and not self._is_chatbot_channel(guild_id, channel_id):
        ...
        async with message.channel.typing():
            reply = await _gemini_reply(content, message.author.display_name, gender, context=context, last_reply=last_reply)
            if not reply or reply == last_reply:
                reply = _fallback_reply(gender, last_reply)
    ...
    async def toggle(self, interaction):
        ...
        await interaction.response.send_message(f"Chatbot is now **{status}** for **{interaction.guild.name}**.", ephemeral=True)
    ...
    async def list_channels(self, interaction):
        ...
        mentions = [
            (interaction.guild.get_channel(int(ch_id)).mention
             if interaction.guild.get_channel(int(ch_id)) else f"`{ch_id}`")
